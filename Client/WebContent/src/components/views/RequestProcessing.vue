							...
							<table class="table table-responsive table-bordered table-striped">
								...
								<tbody>
									<tr v-for="request in requests"
										...
										<td>{{request.tblUserByRequestedBy.tblUserProfile.firstName}} {{request.tblUserByRequestedBy.tblUserProfile.lastName}}</td>
										<td>{{request.tblTransaction.transactionAmount}}</td>
										<td>{{request.tblTransaction.transactionType}}</td>
									</tr>
								</tbody>
							</table>
					  </div>
  ...
  methods: {
	...
	accountFilter(account) {
		...
		approve() {
			...
			const transaction_id = this.selectedAccount.tblTransaction.transactionId;
			const approved_by = store.state.user
			api.request('post','./rest/approveTransaction',{transaction_id,approved_by })
			.then(response =>
			{
		  ...
		  decline() {
			...
			const transaction_id = this.selectedAccount.tblTransaction.transactionId;
			const declined_by = store.state.user
			api.request('post','./rest/declineTransaction',{transaction_id,declined_by })
			.then(response =>
			{
				var responseData = response.data
				if(responseData === "OK")
